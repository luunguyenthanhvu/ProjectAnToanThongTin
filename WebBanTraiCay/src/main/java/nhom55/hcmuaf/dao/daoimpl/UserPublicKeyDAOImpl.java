...
public class UserPublicKeyDAOImpl implements UserPublicKeyDAO {
  ...
  public UserPublicKey getUserPublicKey(int idUser) {
    try {
      ...
      System.err.println(
          "Error fetching UserPublicKey for idUser=" + idUser + ": " + e.getMessage());
      return null;
    }
  }
  ...
  public PublicKey getPublicKey(int idPublicKey) {
    try {
      ...
      System.err.println(
          "Error fetching PublicKey for idPublicKey=" + idPublicKey + ": " + e.getMessage());
      return null;
    }
  }
  ...
  public String getPublicKeyOfUser(int idUser) {
    ...
    try {
      key = JDBIConnector.get().withHandle(h ->
          h.createQuery(
                  "SELECT p.key FROM user_public_key u join public_key p on u.idPublicKey = p.id WHERE idUser = :idUser and status = 'IN_USE'")
              .bind("idUser", idUser)
              .mapTo(String.class)
              .one()
      );
  ...
  public void insertUserPublicKey(int idUser, int idPublicKey, PublicKeyStatus status) {
    try {
      JDBIConnector.get().withHandle(h -> {
        ...
        h.createUpdate(
                "UPDATE user_public_key SET status = :bannedStatus WHERE idUser = :idUser AND status = :inUseStatus")
            .bind("idUser", idUser)
        ...
        return h.createUpdate(
                "INSERT INTO user_public_key (idUser, idPublicKey, status) VALUES (:idUser, :idPublicKey, :status)")
            .bind("idUser", idUser)
            .bind("idPublicKey", idPublicKey)
            .bind("status", status)
            .execute();
      ...
      System.err.println(
          "Error inserting UserPublicKey (idUser=" + idUser + ", idPublicKey=" + idPublicKey + "): "
              + e.getMessage());
    }
  }
  ...
  public int getIdPublicKey(LocalDateTime createDate) {
    try {
      ...
      System.err.println(
          "Error fetching PublicKey ID for createDate=" + createDate + ": " + e.getMessage());
  ...
  public boolean setStatusUserPublicKey(int idUser, int idPublicKey, String status) {
    try {
      JDBIConnector.get().withHandle(h ->
          h.createUpdate(
                  "UPDATE user_public_key SET status = :status WHERE idUser = :idUser AND idPublicKey = :idPublicKey and status = 'IN_USE'")
              .bind("idUser", idUser)
              .bind("idPublicKey", idPublicKey)
              .bind("status", status)
              .execute()
      ...
      System.err.println(
          "Error updating UserPublicKey (idUser=" + idUser + ", idPublicKey=" + idPublicKey + "): "
              + e.getMessage());
      return false;
    }
  }
  ...
  public int insertUserPublicKey(UserPublicKey userPublicKey) {
    return JDBIConnector.get().withHandle(handle -> {
      return handle.createUpdate("INSERT INTO user_public_key(idUser,idPublicKey, status)"
          ...
          .bind("status", userPublicKey.getStatus().getLevel())
  ...
  public UserPublicKey userPublicKey(int userId) {
    ...
    return JDBIConnector.get().withHandle(handle ->
        handle.createQuery(
            ...
            .bind("userId", userId)
            .bind("status",
                PublicKeyStatus.IN_USE.getLevel())
            .mapToBean(UserPublicKey.class)
            .findOne()
            .orElse(null)
    );
