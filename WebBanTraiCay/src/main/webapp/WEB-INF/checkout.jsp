...
<%
    // Lấy giá trị attribute wrongSignature từ /page/order/check-out
    Boolean wrongSignature = (Boolean) request.getAttribute("wrongSignature");
    Boolean errorProcessSignature = (Boolean) request.getAttribute("errorProcessSignature");
%>
...
<script>
  ...
  var hashInfo = document.getElementById("info_hash_for_user");
  var signatureFromUser = document.getElementById("signature_from_user");
  ...
  function validateHash() {
    ...
    if (text.length == 0 || text == null) {
      error.textContent = "Vui lòng không để trống mã hash";
      error.style.display = "block";
      return false;
    } else {
      error.style.display = "none";
  ...
  function validateSignature() {
    ...
    if (text.length == 0 || text == null) {
      error.textContent = "Vui lòng không để trống chữ ký";
      error.style.display = "block";
      return false;
    } else {
      error.style.display = "none";
  ...
  hashInfo.addEventListener("blur", validateHash);
  signatureFromUser.addEventListener("blur", validateSignature);
  ...
  hashInfo.addEventListener("input", validateHash);
  signatureFromUser.addEventListener("input", validateSignature);
  ...
  submitBtn.addEventListener("click", function (event) {
    ...
    if (!isHoValid || !isTenValid || !isDiaChiValid || !isSdtValid || !isEmailValid || !isSoTheValid
        || !isTheCVCValid || !checkedButton.checked || !isHashValid || !isSignatureValid) {
